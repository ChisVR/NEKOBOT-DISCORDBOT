...
client.on("messageCreate", async message => {
    ...
    function getProfileRoles(guildID) {
        return new Promise((resolve, reject) => {
            connection.query(`SELECT isDonator, isPremium, isLegend, isMega, isTwitchSub, isMonthSupporter, isBetaTesters FROM profile WHERE userid = '${guildID}'`, (err, rows) => {
                if (err) return reject(err);
    ...
    const [dbprefix] = await getGuildPrefix(message.guild.id).catch(console.error);
    ...
    if (guildsettingstoggle.globalToggle != 0) {
        if (message.channel.id === guildsettingstoggle.globalchannelid) {
            ...
            client.guilds.cache.forEach(async g => {
                try {
                    const [guildchannelsglobal] = await getGuildToggle(g.id).catch(console.error);
